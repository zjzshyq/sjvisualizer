...
class bar_race(cv.sub_plot):
    ...
    def draw(self, time):
        ...
        if self.invert:
            ...
            print(self._get_data_for_frame(time).head(5))
            data = self._get_data_for_frame(time).sort_values(ascending=False) # 原来的代码
    ...
    def update(self, target_y=0, width=0, value=0, color_data=None):
        if value and self.bar:
            ...
            if self.display_value:
                ...
                ratio_crt = 0.03 #只有数据很大的bar才会在内部显示数据，原来是0.75
                if self.canvas.bbox(self.value)[2] - self.canvas.bbox(self.value)[0] < ratio_crt * width:
                ...
                # when the value should be drawn outside of the bar
                    ...
                    if self.img:
                        self.canvas.coords(self.value, self.x + width + 0.5 * self.size + self.image_obj.width(),
                                           self.y + (self.size) / 3 + (1 - self.font_scale) * self.size / 2)
                    else:
                        self.canvas.coords(self.value, self.x + width + 0.25 * self.size,
                                           self.y + (self.size) / 3 + (1 - self.font_scale) * self.size / 2)
